...
class MoveViewModelTest {
    ...
    val instantTaskExecutorRule =
        InstantTaskExecutorRule() // Forces LiveData to execute on main thread
    ...
    // Mock DAOs
    private val moveDao = mockk<MoveDao>(relaxed = true)
    ...
    private val allTagsLiveData = MutableLiveData<List<MoveTag>>()
    ...
    fun setup() {
        ...
        every { db.moveDao() } returns moveDao
        ...
        every { moveDao.getAllMovesWithTags() } returns movesWithTagsLiveData
        every { moveDao.getAllTags() } returns allTagsLiveData
    ...
    fun `generateStructuredCombo returns valid moves when matching tags found`() = runTest {
        // Given
        val moveTagToprock = MoveTag("t1", "Toprock")
        val moveTagFootwork = MoveTag("t2", "Footwork")

        val moveToprock = Move("m1", "Indian Step")
        val moveFootwork = Move("m2", "6-Step")

        val moveWithTags1 = MoveWithTags(moveToprock, listOf(moveTagToprock))
        val moveWithTags2 = MoveWithTags(moveFootwork, listOf(moveTagFootwork))
        ...
        val requestedSequence = listOf(moveTagToprock, moveTagFootwork)
    ...
    fun `generateStructuredCombo skips moves if no match for tag`() = runTest {
        ...
        val moveTagToprock = MoveTag("t1", "Toprock")
        val moveTagFreeze =
            MoveTag("t3", "Freeze") // We have no freeze moves in our mock data
        ...
        val requestedSequence = listOf(moveTagToprock, moveTagFreeze)
        val result = viewModel.generateStructuredCombo(requestedSequence)
    ...
    fun `getFlashcardMove respects excluded tags`() = runTest {
        ...
        movesWithTagsLiveData.value = listOf(
            MoveWithTags(moveToprock, listOf(moveTagToprock)),
            MoveWithTags(moveFootwork, listOf(moveTagFootwork))
        )
        advanceUntilIdle()

        val targetTags = setOf(moveTagFootwork)
    ...
    fun `getAppDataForExport collects data from all DAOs`() = runTest {
        ...
        // Mock DAO returns
        coEvery { moveDao.getAllMovesList() } returns moves
        coEvery { moveDao.getAllTagsList() } returns moveTags
        coEvery { moveDao.getAllMoveTagCrossRefsList() } returns emptyList()
        coEvery { savedComboDao.getAllRawSavedCombos() } returns savedCombos
        coEvery { battleDao.getAllBattleCombosWithTagsList() } returns battleCombosWithTags
        coEvery { battleTagDao.getAllBattleTagsList() } returns emptyList()
        coEvery { battleDao.getAllBattleComboTagCrossRefs() } returns emptyList()
    ...
    fun `importAppData clears DB and inserts all data`() = runTest {
        ...
        // Verify Inserts
        coVerify { moveDao.insertAllMoves(match { it.size == 1 && it[0].name == "NewMove" }) }
        coVerify { battleDao.insertBattleCombo(match { it.description == "NewBattle" }) }
    ...
    fun `generateComboFromTags allows exceeding pool size when repeats are enabled`() = runTest {
        ...
        val moves = listOf(
            MoveWithTags(move1, listOf(moveTag)),
            MoveWithTags(move2, listOf(moveTag))
        )
        ...
        val result = viewModel.generateComboFromTags(
            selectedMoveTags = setOf(moveTag),
            length = 50,
            allowRepeats = true
        )
    ...
    fun `generateComboFromTags clamps length to max limit`() = runTest {
        ...
        val moves = listOf(
            MoveWithTags(move1, listOf(moveTag)),
            MoveWithTags(move2, listOf(moveTag)),
            MoveWithTags(move3, listOf(moveTag))
        )
        movesWithTagsLiveData.value = moves
        advanceUntilIdle()

        // Case 1: Request 3 (Exact) - No Repeats
        val result3 = viewModel.generateComboFromTags(setOf(moveTag), 3, false)
        assertEquals(3, result3.size)

        // Case 2: Request 2 (Under) - No Repeats
        val result2 = viewModel.generateComboFromTags(setOf(moveTag), 2, false)
        assertEquals(2, result2.size)

        // Case 3: Request 60 (Over Max) - Repeats Enabled
        // FIX: Logic clamps to 6.
        val result60 = viewModel.generateComboFromTags(setOf(moveTag), 60, true)
