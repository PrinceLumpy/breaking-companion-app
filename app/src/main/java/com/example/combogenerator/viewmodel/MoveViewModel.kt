...
import com.example.combogenerator.data.AppDB
...
import com.example.combogenerator.data.transfer.AppDataExport // Required import
...
class MoveViewModel(application: Application) : AndroidViewModel(application), IMoveViewModel {
    ...
    init {
        // Logging for LiveData updates (can be removed or conditionalized for production)
        movesWithTags.observeForever { Log.d("MoveViewModelLifecycle", "movesWithTags updated: ${it.size} items") }
        allTags.observeForever { Log.d("MoveViewModelLifecycle", "allTags updated: ${it.size} items") }
        savedCombos.observeForever { Log.d("MoveViewModelLifecycle", "savedCombos updated: ${it.size} items") }
    }
    ...
    override fun addMove(moveName: String, selectedTags: List<Tag>) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                Log.d("MoveViewModel", "addMove successful for: $moveName")
            } catch (e: Exception) {
    ...
    override fun addTag(tagName: String) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                Log.d("MoveViewModel", "addTag successful for: $tagName")
            } catch (e: Exception) {
    ...
    override fun updateMoveAndTags(moveId: String, newName: String, newSelectedTags: List<Tag>) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                Log.d("MoveViewModel", "updateMoveAndTags successful for id: $moveId")
            } catch (e: Exception) {
    ...
    override fun deleteMove(move: Move) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                Log.d("MoveViewModel", "deleteMove successful for: ${move.name}")
            } catch (e: Exception) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            try {
                if (newName.isNotBlank()) {
                    ...
                    Log.d("MoveViewModel", "updateTag successful for id: $tagId")
                } else {
    ...
    override fun deleteTag(tag: Tag) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                Log.d("MoveViewModel", "deleteTag successful for: ${tag.name}")
            } catch (e: Exception) {
    ...
    override fun saveCombo(comboName: String, moves: List<Move>) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                Log.d("MoveViewModel", "saveCombo successful for: $finalComboName")
            } catch (e: Exception) {
    ...
    override fun deleteSavedCombo(savedComboId: String) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                Log.d("MoveViewModel", "deleteSavedCombo successful for id: $savedComboId")
            } catch (e: Exception) {
    ...
    override fun updateSavedComboName(savedComboId: String, newName: String) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                Log.d("MoveViewModel", "updateSavedComboName successful for id: $savedComboId")
            } catch (e: Exception) {
    ...
    override fun resetDatabase() {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                Log.d("MoveViewModel", "Database reset and default tags re-populated.")
            } catch (e: Exception) {
    ...
    override suspend fun getAppDataForExport(): AppDataExport? = withContext(Dispatchers.IO) {
        try {
            ...
            Log.d("MoveViewModel", "getAppDataForExport successful.")
    ...
    override suspend fun importAppData(appData: AppDataExport): Boolean = withContext(Dispatchers.IO) {
        try {
            ...
            Log.d("MoveViewModel", "importAppData successful.")
            true
        } catch (e: Exception) {
    ...
    override fun onCleared() {
        super.onCleared()
        Log.d("MoveViewModelLifecycle", "MoveViewModel cleared.")
    }
}
...
class FakeMoveViewModel : IMoveViewModel {
    private val initialFakeMovesWithTags = listOf(
        ...
        MoveWithTags(Move("m2", "Preview Kick"), listOf(Tag("t1", "Starter"), Tag("t2", "Finisher")))
    )
    ...
    private val initialFakeSavedCombosWithMoves = listOf(
        SavedComboWithMoves(SavedCombo(id = "sc1", name = "My Favorite Combo"), listOf(fakeMove1, fakeMove2)),
        SavedComboWithMoves(SavedCombo(id = "sc2", name = "Quick Warmup"), listOf(fakeMove3, fakeMove1, fakeMove3))
    )
    ...
    override suspend fun getAppDataForExport(): AppDataExport? {
        ...
        val savedComboMoveLinks = _savedCombos.value?.flatMap { scwm ->
            scwm.moves.mapIndexed { index, move -> SavedComboMoveLink(scwm.savedCombo.id, move.id, index) }
        } ?: emptyList()
